...
def _resolve_player_name(player: dict[str, Any]) -> str | None:
    """
    API-Football 표기: name 우선, 없으면 firstname + lastname.
    공백·제어문자 정리, 비어 있으면 스킵.
    """
    raw = player.get("name")
    label = ""
    if isinstance(raw, str) and raw.strip():
        ...
        fn = str(player.get("firstname") or "").strip()
        ln = str(player.get("lastname") or "").strip()
        label = f"{fn} {ln}".strip()
    if not label:
        return None
    label = "".join(ch for ch in label if ch == "\t" or ord(ch) >= 32).strip()
    label = _WS_RE.sub(" ", label).strip()
    if not label or len(label) > 200:
        return None
    low = label.lower()
    if low.startswith("u25 player") or low == "unknown" or low == "-":
        return None
    return label
...
def _clean_geo_label(value: object) -> str:
...
def _resolve_player_id(player: dict[str, Any]) -> str | None:
    """API-Football numeric id only; invalid rows are skipped upstream."""
    pid = player.get("id")
    if pid is None:
        return None
    s = str(pid).strip()
...
def _resolve_nationality(player: dict[str, Any], league_country: str) -> str:
    """
    Prefer player nationality from the player object (true 국적),
    then birth.country, then league country from harvest context.
    """
    nat = _clean_geo_label(player.get("nationality"))
    if nat:
        return nat
    birth = player.get("birth") if isinstance(player.get("birth"), dict) else {}
    birth_country = _clean_geo_label(birth.get("country"))
    if birth_country:
        return birth_country
    league = _clean_geo_label(league_country)
    return league if league else "Unassigned"


def _resolve_player_age(player: dict[str, Any], *, as_of: date) -> int | None:
    """Use API age when present; otherwise derive from birth.date for stable U25 filtering."""
    raw = player.get("age")
    if raw is not None and str(raw).strip() != "":
        try:
            ...
            a = None
        if a is not None and 0 <= a < 25:
            return a
    birth = player.get("birth") if isinstance(player.get("birth"), dict) else {}
    d = str(birth.get("date") or "").strip()
    if len(d) >= 10:
        try:
...
def _to_snapshot(
    *,
    ...
    if not pid:
        return None
    age = _resolve_player_age(player, as_of=as_of)
    if age is None:
        return None
    pname = _resolve_player_name(player)
    if not pname:
        return None
    ...
    normalized_country = _resolve_nationality(player, country)
