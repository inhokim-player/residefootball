...
const LOCK_TIMEOUT = 6; // 분 (Vercel Pro maxDuration 5분보다 1분 여유)
...
function pagedUrl(base, page) {
  ...
  function capByTeam(items, max) {
    ...
    const out = [];
    teamMap.forEach(players => {
      out.push(...players.slice(0, max));
    });
    return out;
  }
...
export default async function handler(req, res) {
  ...
  const targetPath    = String(process.env.REGISTERED_PLAYERS_FILE_PATH   || "data/u25-players.json").trim();
  ...
  // ── 강제 락 해제 모드 (?force_unlock=true) ─────────────────
  const forceUnlock = (() => {
    try {
      const raw = String(req.url || "");
      const qi  = raw.indexOf("?");
      if (qi < 0) return false;
      return new URLSearchParams(raw.slice(qi + 1)).get("force_unlock") === "true";
    } catch (_) { return false; }
  })();

  if (forceUnlock) {
    console.log("[lock] 강제 해제 요청");
    await releaseLock(ghToken, ghRepo, ghBranch);
    return res.status(200).json({
      ok: true,
      mode: "force_unlock",
      message: "락 파일을 강제 해제했습니다. 이제 정상 실행 가능합니다.",
    });
  }
  ...
  try {
    ...
    // ── U25 필터 + 나이 23세 이하로 축소 ─────────────────────
    const hardMaxAge = Math.min(maxAge, 23);
    const u25raw = filterU25(allItems, hardMaxAge);

    // ── 팀당 최대 22명 ────────────────────────────────────────
    const u25capped = capByTeam(u25raw, 22);

    // ── 슬림 필드만 저장 ──────────────────────────────────────
    const u25 = u25capped.map(slimPlayer);

    console.log(`[summary] 전체:${allItems.length} / U${hardMaxAge}:${u25raw.length} / 팀캡:${u25.length}`);
    ...
    return res.status(200).json({
      ...
      players:          u25.length,
      rows_total:       allItems.length,
      rows_u23:         u25raw.length,
      rows_after_cap:   u25.length,
      season,
      max_age:          hardMaxAge,
      path:             targetPath,
