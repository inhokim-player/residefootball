...
_ORIGIN_RE = re.compile(
    r"^https?://[A-Za-z0-9.\-_]{1,253}(?::\d{1,5})?$|^null$",
    re.IGNORECASE,
)


def _sanitize_cors_origin(value: str) -> str | None:
    s = sanitize_query_value(value, max_len=256).strip()
    if not s:
        return None
    if not _ORIGIN_RE.match(s):
        return None
...
def get_cors_allow_origins() -> list[str]:
    ...
    try:
        ...
        if isinstance(origins, list) and origins:
            merged: list[str] = []
            for x in origins:
                ok = _sanitize_cors_origin(str(x))
                if ok:
                    merged.append(ok)
            if merged:
                return merged
...
def _allow_local_token_bypass() -> bool:
    ...
    # 기본은 코드·automation_settings 의 False(운영 안전). 토큰이 없으면 미들웨어가 꺼져 있어 사실상 무관.
    return bool(sec.get("allow_local_token_bypass", False))
...
class APITokenMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Any) -> Response:
        ...
        if path == "/health":
            return await call_next(request)
        # 로컬 우회는 반드시 실제 TCP 피어(프록시 X-Forwarded-For 신뢰 안 함).
        client_host = ((request.client.host if request.client else "") or "").strip()
        # 로컬 개발 편의. 운영에서는 automation_config·환경변수로 우회를 명시적으로만 허용.
...
def _client_ip_for_rate_limit(request: Request) -> str:
    """
    기본은 TCP 클라이언트 IP. 리버스 프록시 뒤에서는 U25_TRUST_PROXY=1 일 때만
    X-Forwarded-For 의 첫 홉을 사용(신뢰할 수 있는 프록시에서만 켜세요).
    """
    if (os.environ.get("U25_TRUST_PROXY") or "").strip() == "1":
        xff = (request.headers.get("x-forwarded-for") or "").strip()
        if xff:
            first = xff.split(",")[0].strip()
            if first:
                return first[:128]
    return ((request.client.host if request.client else "") or "").strip() or "unknown"


class BodySizeLimitMiddleware(BaseHTTPMiddleware):
    """POST 등 본문이 과도하게 큰 요청을 거부(메모리·JSON 폭탄緩和)."""

    def __init__(self, app: Any) -> None:
        super().__init__(app)
        sec = _read_security_cfg()
        try:
            cap = int(sec.get("max_request_body_bytes", 65536) or 65536)
        except Exception:  # noqa: BLE001
            cap = 65536
        self._max_bytes = max(1024, min(cap, 2_097_152))
