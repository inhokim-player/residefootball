...
def main() -> int:
    ...
    if bool(sec.get("allow_local_token_bypass", False)):
        _warn(
            "security.allow_local_token_bypass is true — OK for 집 PC/로컬(127.0.0.1). "
            "인터넷에 공개하는 서버에서는 false 로 두고 U25_API_TOKEN 을 반드시 적용하세요."
        )
        warns += 1
    else:
        _ok("allow_local_token_bypass is false (public-server friendly)")
